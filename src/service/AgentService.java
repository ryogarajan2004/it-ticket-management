...
public class AgentService {
    ...
    private static User requireAgent() {
        ...
        if (!u.getRole().equals(Role.AGENT)) throw new SecurityException("Agent access required");
        return u;
    }
    ...
    public static void autoAssign(Ticket ticket) {
        for (User u : Main.store.users) {
            if (u instanceof Agent agent) {
                if (agent.getCategory() == (ticket.getCategory())) {
                    ticket.reassign(agent.getId());
    ...
    private static Ticket findAssignedTicket(Long id, User agent) {
        return Main.store.tickets.stream().filter(t -> t.getId().equals(id)).filter(t -> agent.getId().equals(t.getAssigneeId())).findFirst().orElseThrow(() -> new IllegalArgumentException("Ticket Not assigned to you"));
    }

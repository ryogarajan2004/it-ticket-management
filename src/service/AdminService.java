...
public class AdminService {
	...
	public static void changeUserRole(Long userId, Role newRole, Category category) {
		...
		User existing = Main.store.users.stream().filter(u -> u.getId().equals(userId)).findFirst()
		...
		if (newRole == Role.AGENT) {
			...
			updated = new User(existing.getId(), existing.getName(), existing.getUsername(), existing.getEmail(),
					existing.getPassword(), Role.AGENT, category);

		} else {
			updated = new User(existing.getId(), existing.getName(), existing.getUsername(), existing.getEmail(),
					existing.getPassword(), newRole, null);
		}
	...
	private static Ticket findTicket(Long id) {
		return Main.store.tickets.stream().filter(t -> t.getId().equals(id)).findFirst()
